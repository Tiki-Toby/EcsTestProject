...
namespace XFlow.Net.Client.Socket
{
    public class WebSocketConnection
    {
        ...
        private void OnOpen(WebSocket s)
        {
            ...
            socket.Send(P2P.P2P.BuildRequest(Address, Request));

            if (Address == P2P.P2P.ADDR_BROADCAST)
            {
        ...
        private void OnBinary(WebSocket webSocket, byte[] data)
        {
            try
            {
                ...
                var errAddr = "";
                if (P2P.P2P.CheckError(data, out errAddr))
                {
                    Completed(true);
                    return;
                }

                Response = P2P.P2P.ParseResponse<Packet>(data);
                Completed(false);
            }
            catch (Exception e)
            {
